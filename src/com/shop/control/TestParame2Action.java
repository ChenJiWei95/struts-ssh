...
public class TestParame2Action extends SuperActionSupport{
	private static final long serialVersionUID = 7539369474585568995L;
	
	private static Logger log = Logger.getLogger(TestParame2Action.class); // 日志对象 
	
	// 此处获取参数的方式为对象驱动，请求方式为get
	// 表单的写法 name=user.id， name=user.username
	...
	public void saveorupdate2(HttpServletRequest request){
		log.info("反射执行saveorupdate2");
		request.setAttribute("text", "对象驱动");
	}	
